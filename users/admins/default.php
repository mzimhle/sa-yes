...
if(isset($_GET['action']) && trim($_GET['action']) == 'sendlogin') {
	...
	if($response['result'] == true && $response['error'] == '') {
		...
		$mentorData = $userObject->adminLogin(trim($_REQUEST['usercode']));
		
		if($mentorData) {
		
			/* Send email to the user. */												
			$mentorData['category']		= 'admin-login';			
...
if((isset($_GET['action']) && trim($_GET['action']) == 'searchusers') && !isset($_REQUEST['cvs'])) {
	...
	$userData = $userObject->searchAdmin($where, 'user_name asc');
...
$smarty->display('users/admins/default.tpl');
