...
function downloadcvs() {
	...
	window.location = '/reports/meetings/?action=searchmeetings&cvs=1&from='+from+'&meetingstatus='+meetingstatus+'&smenteeid='+smenteeid+'&smentorid='+smentorid+'&to='+to+'&type='+type+'&withstaff='+withstaff+'&mentorship='+mentorship;;
		...
		function(data) {
			if(data.result) {
				...
				for(var i = 0; i < data.records.length; i++) {
					...
					html += '<td valign="top">' + item.meeting_date + '</td>';
			...
			tblhtml = '<table id="dataTable" class="display" style="table-layout: fixed"><thead><tr><th></th><th>Mentorship</th><th>Created Date</th><th>Meeting Date</th><th>Start Time</th><th>Mentor Name</th><th>Mentee Name</th><th>Meeting Status</th><th>Reason</th><th>Type</th><th>With Staff</th></tr></thead><tbody id="dataTableContent">'+html+'</tbody></table>';
