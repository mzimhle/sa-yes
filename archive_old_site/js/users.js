...
$(document).ready(function() {
	...
	// init data tables
	var tableId = 'usersMentor';
	insertDetailsColumn(tableId);
	mentorsTable = $('#' + tableId).dataTable({
		"sDom" : 'T<"clear">lfrtip',
		"oTableTools" : {
		...
		"aoColumnDefs": [
			...
			{ "sWidth": "12em", "aTargets" : [1,2] },
			{ "sWidth": "15em", "aTargets" : [3] },
			{ "sWidth": "13em", "aTargets" : [4,5] },
			{ "sWidth": "10em", "aTargets" : [6] }
		],
		"bAutoWidth":false
	});
	fnEnableSelection(mentorsTable);
	initDetailsListeners(tableId,mentorsTable);
	...
	jQuery("#mentorMenteeEditForm").validate({
...
function fnFormatDetails ( oTable, nTr )
{
    ...
    var notes = '';
    if (oTable == mentorsTable) {
    	notes = aData[7];	
    } else if (oTable == menteesTable) {
    	notes = aData[10];
    } else if (oTable == adminsTable) {
    	notes = aData[6];
    }
    ...
    sOut += '<tr><td class="detailsHeader">Notes:</td><td>'+notes+'</td></tr>';
    sOut += '</table>';
     
    return sOut;
}

function initMenteeTab() {
	// init data table
	var tableId = 'usersMentee';
	insertDetailsColumn(tableId);
	menteesTable = $('#' + tableId).dataTable({
		"sDom" : 'T<"clear">lfrtip',
		"oTableTools" : {
			"sSwfPath" : "js/extras/TableTools/media/swf/copy_csv_xls_pdf.swf"
		},
		...
		"aoColumnDefs": [
			...
			{ 'sWidth': '2em', 'aTargets' : [0] },
			{ "sWidth": "15em", "aTargets" : [3] },
			{ "sWidth": "20%", "aTargets" : [9] }
		],
		"bAutoWidth":false
	});
	fnEnableSelection(menteesTable);	
	initDetailsListeners(tableId,menteesTable);
...
function initAdminTab() {
	// init data table
	var tableId = 'usersAdmin';
	insertDetailsColumn(tableId);
	adminsTable = $('#' + tableId).dataTable({
		"sDom" : 'T<"clear">lfrtip',
		"oTableTools" : {
			"sSwfPath" : "js/extras/TableTools/media/swf/copy_csv_xls_pdf.swf"
		},
	...
	initDetailsListeners(tableId,adminsTable);
...
function editClicked(userType) {
	...
	// remove the details icon column
...
function updateUser(formName) {
	...
	if (formName == "editMentor") {
		...
		values['selective'] = 'true';
	} else if (formName == "menteeEditForm") {
		values = getEditValues_MenteeUpdate();
		values['selective'] = 'true';
	}
...
function getEditValues_Mentor(ignoreEmpty) {
	...
	var notes = document.forms['editMentor']['notes'].value;
	values['notes'] = notes;
	return values;
}
...
function getEditValues_Mentee() {
	...
	var race = document.forms['editMentee']['race'].value
	values['race'] = race;
	...
	var notes = document.forms['editMentee']['notes'].value;
	values['notes'] = notes;
...
function getEditValues_Admin() {
	...
	var notes = document.forms['editAdmin']['notes'].value;
	values['notes'] = notes;
...
/**
 * Returns the values from the mentor's mentee edit form. Values are a returned 
 * as an object where the property names are the parameter names expected by the 
 * server in the update user call.
 */
function getEditValues_MenteeUpdate() {
	
	var values = new Array();
	values['userType'] = 'mentee';
	
	var userId = document.forms['menteeEditForm']['userId'].value;
	values['id'] = userId;
	
	var home = document.forms['menteeEditForm']['home'].value;
	values['home'] = home;	
	
	var homeDetails = document.forms['menteeEditForm']['homeDetails'].value;
	values['homeDetails'] = homeDetails;
...
function updateUserSuccess(data,formName) {
	if (data.error) {
		...
		if (formName == "editMentor") {
			...
			values = getEditValues_Mentee();
			order = ['fname','lname','email','phone','cphone','race','home','homeDetails','date','notes'];
			table = menteesTable;
		} else if (formName == "editAdmin") {
			values = getEditValues_Admin();
			...
			values = getEditValues_MenteeUpdate();
			order = ['home','homeDetails'];
			table = null;
			
			$("#message").text('Mentee information updated successfully.');
			$("#message").show();
		}
...
function updateUserTable(table, updated) {
	...
	selected = selected[0];
	var index = table.fnGetPosition(selected);
...
function addUser(formName) {
	...
	if(formName.indexOf("Mentor") > -1) {
		...
		var race = document.forms[formName]['race'].value;
		data += "&race=" + race;
...
function addUserSuccess(data) {
	...
	if(data.error) {
		...
		var user = data.user;
		user.splice(0,0,'<img src="' + detailsOpen + '"/>');
		
		// add new row
		var oSettings = $('#' + data.tableId).dataTable().fnSettings();
		var aiAdded = $('#' + data.tableId).dataTable().fnAddData(user);
