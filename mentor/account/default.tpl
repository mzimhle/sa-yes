      ...
      <form id="detailsForm" name="detailsForm" action="/mentor/account/" method="post">
        <table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" class="form">  
		 <tr>
			...
			<td><input type="text" name="user_name" id="user_name" value="{$userData.user_name}" size="30"/></td>
            <td class="left_col{if isset($errorArray.user_surname)} error{/if}"><h4>Surname:</h4></td>
			<td><input type="text" name="user_surname" id="user_surname" value="{$userData.user_surname}" size="30"/></td>			
          </tr>	
          <tr>
            <td class="left_col{if isset($errorArray.user_idnumber)} error{/if}"><h4>ID Number:</h4></td>
			<td><input type="text" name="user_idnumber" id="user_idnumber" value="{$userData.user_idnumber}" size="30"/><br />{if isset($errorArray.user_idnumber)} - <span style="color: red">{$errorArray.user_idnumber}</span>{/if}</td>
          ...
          <tr>
            <td colspan="4" class="left_col{if isset($errorArray.area_code)} error{/if}"><h4>Area:</h4><br />
				<input type="text" name="area_name" id="area_name" value="{$userData.area_shortPath}" size="80" />
          ...
          <tr>
			...
			<td><input type="text" name="user_email" id="user_email" value="{$userData.user_email}" size="30"/><br /> {if isset($errorArray.user_email)}<span style="color: red">{$errorArray.user_email}</span>{/if}</td>
          ...
          <tr>
			...
			<td><input type="password" name="user_password" id="user_password" value="{$userData.user_decoded}" size="30"/><br /> {if isset($errorArray.user_password)}<span style="color: red">{$errorArray.user_password}</span>{/if}</td>
            <td class="left_col{if isset($errorArray.user_password2)} error{/if}"><h4>Retype Password:</h4></td>
			<td><input type="password" name="user_password2" id="user_password2" value="{$userData.user_decoded}" size="30"/><br /> {if isset($errorArray.user_password2)}<span style="color: red">{$errorArray.user_password2}</span>{/if}</td>			
          </tr>		  
          <tr>
            <td class="left_col{if isset($errorArray.user_cell)} error{/if}"><h4>Cell:</h4></td>
			<td><input type="text" name="user_cell" id="user_cell" value="{$userData.user_cell}" size="30"/><br />E.g. 0734897584 {if isset($errorArray.user_cell)} - <span style="color: red">{$errorArray.user_cell}</span>{/if}</td>
            <td class="left_col{if isset($errorArray.user_telephone)} error{/if}"><h4>Telephone:</h4></td>
			<td><input type="text" name="user_telephone" id="user_telephone" value="{$userData.user_telephone}" size="30"/><br />E.g. 0215874698 {if isset($errorArray.user_telephone)} - <span style="color: red">{$errorArray.user_telephone}</span>{/if}</td>			
          </tr>
          <tr>
            <td colspan="4" class="left_col{if isset($errorArray.user_notes)} error{/if}" valign="top"><h4>Notes:</h4><br />
			<textarea name="user_notes" id="user_notes"  cols="150" rows="20">{$userData.user_notes}</textarea></td>
          </tr>			  
        </table>
      </form>
	</div>
